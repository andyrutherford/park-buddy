    ...
    fetchPark(parkId)
      .then((res) => {
        setParkInfo({
          ...
          description: res.description ? res.description : 'No description available.',
          location: {
            lng: res.longitude,
            lat: res.latitude,
          },
          images: res.images,
          activities: res.activities ? res.activities.map((a) => a.name).slice(0, 20) : 'No activities available.',
          ...
          entranceFee: {
            cost: res.entranceFees[0] ? '$' + parseInt(res.entranceFees[0].cost).toFixed(2) : 'Price unknown',
            description: res.entranceFees[0] ? res.entranceFees[0].description : 'No information about entrance fees is available.',
          },
          address: res.addresses[0]? res.addresses[0].city + ', ' + res.addresses[0].stateCode : 'Location is unavailable',
          weather: res.weatherInfo ? res.weatherInfo : 'No weather information available.',
          operatingHours: {
            description: res.operatingHours[0].description,
            days: res.operatingHours[0].standardHours,
          },
          directions: {
            info: res.directionsInfo ? res.directionsInfo : 'Directions for this park are not available.',
            url: res.directionsUrl ? res.directionsUrl : 'https://www.nps.gov/index.htm',
          },
        });
        setLoading(false);
      })
